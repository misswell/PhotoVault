...
- 🔴 **系统发起的退出必须兜底复位状态（实测 P0）**：下拉提交后 UIKit 自己结束呈现，`dismissIfNeeded` 的 completion 和 delegate 都可能不走——若不复位，屏幕上 `viewerRequest` 残留非 nil、`isViewerTransitioning` 卡在 true、网格 `collectionView.isUserInteractionEnabled = false`，表现为"查看器明明退出了，之后点什么都没反应"。兜底是 `PhotoViewerHostingController.viewDidDisappear`（bridge 内子类）→ `hostedViewDidVanish()`：与 programmatic 路径、`presentationControllerDidDismiss` 三路信号全部幂等（`presentedRequestID` 守卫），谁先到谁复位。
- 🔴 **退出动画没结束也必须能处理下一步操作（提交即唤醒）**： dismissal transition 一开始（`viewWillDisappear` + `isBeingDismissed`）就做三件事——网格立即恢复交互（`onDismissalCommitted` → `isViewerTransitioning = false`）、把转场容器（UITransitionView）`isUserInteractionEnabled = false` 让触摸穿透到网格、新打开请求进入排队（UIKit 不允许在同一 presenter 的退出动画中 present，`flushPendingRequest` 用主队列重试直到 `presentedViewController` 清空）。新请求靠**会话 id 超替**存活：`onDismissed` 携带被退出的会话 id，屏幕的清理只在 id 匹配时执行，动画中点击的新请求不会被旧退出的回调冲掉。
- 🔴 **`viewWillDisappear` ≠ 提交**：交互式下拉在**拖动开始**就触发它（取消时再触发 `viewWillAppear` 回来）——所以 began 钩子里禁用容器触摸是安全的（进行中的手势不被打断），但必须配对取消恢复（`viewWillAppear` + began 标记 → 容器交互恢复、网格重新休眠），否则一次取消的下拉会留下"看着在、摸不着"的死查看器。唤醒网格触发的 SwiftUI 更新会让 dismissing 会话自己的请求再次流经 `sync`——超替分支必须按 `request.id != presentedRequestID` 过滤，否则同一个查看器会在退出后又弹回来。began 钩子里对容器交互的禁用要**持续再断言**约 0.8 秒（`interactionReassertTimer`）：UIKit 在拖动提交的转场装配阶段可能把容器重新打开，一次性禁用会被无声覆盖。
