...
- 🔴 **下拉方向仲裁只回答"这笔触摸该不该从横向分页器手里拿走"，不回答"退出转场结束了没有"**：`PhotoViewerTransitionState.interactiveDismissState`（idle/dragging/cancelling/committed）由 `dismissalPhaseChanged(from:)` 这唯一写路径镜像，`ViewerDownwardIntentGesture` 经 `mayArbitrateDownwardDrag` 读它——**`.idle` 和 `.cancelling` 都算开**，只有 `.dragging`（系统已经在拖这笔下拉）和 `.committed`（正在退出）才关。理由：仲裁器最多只能把拖动从**横向** pager 那里拿走，而向下的拖动本来就翻不动横向 pager，所以"关门"的收益恒为零，代价是一整笔下拉消失。⚠️ 旧版写成"只认 `.idle`"（`canBeginDownwardIntent`），就是用户报的"第一次下拉取消后马上再下拉没反应"，别再改回去。
...
- TestFlight 上传使用 **App Store Connect API 密钥**（Xcode 的 Apple ID 会话几天就过期，"Failed to Use Accounts" 即此原因；API 密钥无有效期）：
  - 本工作区直接用全局脚本，**必须把手动签名配置作为第二个参数传进去**：`~/.appstoreconnect/upload-testflight.sh build/PhotoVault-NN.xcarchive build/ExportOptions-manual.plist`（所有项目、所有 AI 工具通用；详见 `~/.appstoreconnect/README.md` 与 `~/.codex/AGENTS.md`）。
    🔴 **只传归档路径会在 `-exportArchive` 的 `IDEDistributionAnalyzeVersionStep` 静默挂死**（2026-09-22 实测：进程 0% CPU、无 socket、17 分钟不动，日志停在 `PublicAppsService: fetched 1 items`）——脚本缺省走 `signingStyle=automatic`，而本项目归档依赖 Xcode 云端签名，API 密钥无权限，它既不报错也不返回。判活方法：`ps -o stat,%cpu -p <xcodebuild pid>` + 看 `IDEDistribution*.log` 的最后时间戳；卡住就直接杀掉重来手动路线，别等。
    ⚠️ `build/` 是 gitignore 的，`ExportOptions-manual.plist` **会随清理丢失**；丢了就按下面的证书 SHA-1 与描述文件名重建一份（`manageAppVersionAndBuildNumber=false`，build 号自己管）。
  - 上传成功（`Upload succeeded` + `EXPORT SUCCEEDED`）**还不算完**：`processingState` 要从 `PROCESSING` 等到 `VALID`（Apple 侧通常几分钟）才可用。轮询脚本必须**显式带 bundle id**：`python3 ~/.appstoreconnect/publish-build.py <build号> --bundle-id com.misswell.PhotoVault`——它的 `--bundle-id` 缺省是 `com.misswell.SnapIn`（另一个项目），不传就会在错误的 App 上找不到 build、每 30 秒重试一次白等 30 分钟（2026-09-22 踩过）。
  - 🔴 **内部测试组不需要、也不允许手动分配 build**：该组 `hasAccessToAllBuilds: true`，上传即对内部测试者可见。`POST /v1/betaGroups/{id}/relationships/builds` 会返回 422 `Cannot add internal group to a build.`——这不是失败，是"没必要"。⚠️ 别被 `GET /v1/betaGroups/{id}/builds` 误导：它只列显式分配过的 build（本项目里只到 28，29~34 都不在里面但内部测试照样能拿到）。只有公开链接/外部组才需要显式分配。
  - 🔴 **真正会卡住 TestFlight 的是导出合规**：归档 Info.plist 没写 `ITSAppUsesNonExemptEncryption` 时，`GET /v1/builds` 的 `usesNonExemptEncryption` 为 `null`，即 ASC 的 "Missing Compliance"，build 不下发给测试者。实测 29~33 全是 `null`（28 是在网页上手动答过才为 `false`）。已在 `PhotoVault/Info.plist` 写死 `<false/>`（该 target `GENERATE_INFOPLIST_FILE=NO`，键必须落在这个文件里）；对已上传的 build 可直接补：`PATCH /v1/builds/{id}`，body `{"data":{"type":"builds","id":"<uuid>","attributes":{"usesNonExemptEncryption":false}}}`，改完字段立刻翻成 `false`，无需网页操作。
  - 密钥：当前默认是 **`5KDZSQND5Q`（photovault-cli-air，Admin 角色，可建 App/描述文件/上传）**；`248D8U8C36`（photovault-cli）是旧默认，只能上传。Issuer ID `102e47ab-8e2a-4204-b82b-200d5287f267`，.p8 位于 `~/.appstoreconnect/private_keys/`（私有凭据，严禁提交进仓库或外发）；环境变量已配入 `~/.zshenv`，脚本内也各有一份缺省值。
  - 本机分发证书：`Apple Distribution: Guofeng Liu (U8U443D7ZL)`，SHA-1 `1F09B93C9C602DC08B42E84336669C5F3954302D`（2026-09-06 经开发者门户 CSR 流程创建，私钥在登录钥匙串）；App Store 描述文件名 `PhotoVault AppStore 17`。
  - 也可直接调用命令（递增 build → archive 后）：
    ...
    xcodebuild -exportArchive -archivePath build/PhotoVault-NN.xcarchive \
      ...
      -authenticationKeyPath ~/.appstoreconnect/private_keys/AuthKey_5KDZSQND5Q.p8 \
      -authenticationKeyID 5KDZSQND5Q \
